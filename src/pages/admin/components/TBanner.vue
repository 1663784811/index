        ...
        <div class="tableBox scroll">
            ...
            >
                ...
                <template slot-scope="{row}" slot="status">
                    <span v-if="row.status === 0"><Icon type="md-close-circle" class="colorWarning"/></span>
                    <span v-else-if="row.status === 1"><Icon type="md-checkmark-circle" class="colorSuccess"/></span>
                    <span v-if="row.status === 2"><Icon type="md-close-circle" class="colorError"/></span>
                </template>

                <template slot-scope="{row}" slot="img">
                    <img style="max-height: 50px" :src="row.photo" alt="">
                </template>
...
<script>
    ...
    import {
        ...
        TableHeaderJson,
        searchWhereJson
    } from "@/config/jsonObj/tBanner";

    import {
        API_findPageTBanner,
        API_delTBanner,
        API_saveTBanner
    } from "@/config/api/tBanner";
    ...
    export default {
        ...
        methods: {
            ...
            requestTableDataFn: function () {
                ...
                API_findPageTBanner(this.requestTableData).then(res => {
            ...
            submitDataFn: function () {
                ...
                API_saveTBanner(this.submitData).then(() => {
                    this.winShow = false;
            ...
            delIdFn: function (sendObj) {
                API_delTBanner(sendObj).then(responseData => {
